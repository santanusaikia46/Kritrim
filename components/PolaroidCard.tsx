...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, dragConstraintsRef, onShake, onDownload, isDraggable = true }) => {
                ...
                {status === 'done' && imageUrl && (
                    <>
                        <div className={cn(
                            ...
                            isDraggable && "opacity-0 group-hover:opacity-100", // Only hide on hover if it's draggable
                        )}>
                            {onDownload && (
                                <button
                                    onClick={(e) => {
